import { $, api, jsonApi, clearAuthToken, esc, formatDate } from "./common.js";
...
function renderCafeList() {
  ...
  if (!state.cafes.length) {
    $("cafe-list").innerHTML = emptyState("등록된 카페 데이터가 없습니다. 기존 데이터는 CSV 업로드로 먼저 넣어야 합니다.");
    return;
  }
  if (!query) {
    ...
    ? rows.map((cafe) => `
      <div class="item">
        ...
        <div class="btns" style="margin-top:6px;">
          <button type="button" class="pick-cafe" data-id="${esc(cafe.id)}">편집</button>
        </div>
      </div>
    `).join("")
...
function renderSubmissions() {
  ...
  $("sub-list").innerHTML = state.submissions.map((submission) => `
    <div class="item">
      ...
      <div class="mini">상태: ${esc(submission.status)} ${submission.reject_reason ? ` / 반려사유: ${esc(submission.reject_reason)}` : ""}</div>
...
function renderReviewedSubmissions() {
  ...
  $("reviewed-list").innerHTML = state.reviewedSubmissions.map((submission) => `
    <div class="item">
      <h3>${esc(submission.name)} <small style="font-size:.72rem;">(${esc(submission.username)})</small></h3>
      <div class="mini">${esc(submission.address)}</div>
      <div class="mini">처리: ${submission.status === "approved" ? "승인" : "반려"} ${submission.reviewed_by_username ? ` / 담당: ${esc(submission.reviewed_by_username)}` : ""}</div>
...
function renderErrorReports() {
  ...
  $("error-list").innerHTML = state.errorReports.map((report) => `
    <div class="item">
      ...
      <div class="mini">상태: ${report.status === "open" ? "확인 대기" : "처리 완료"}${report.resolved_by_username ? ` / 처리: ${esc(report.resolved_by_username)}` : ""}${report.resolved_at ? ` / ${esc(formatDateTime(report.resolved_at))}` : ""}</div>
      <div class="reply-block">
        ...
        <textarea class="error-reply-input" data-id="${esc(report.id)}" placeholder="이 오류 제보에 대한 답변을 입력하세요.">${esc(report.reply_message || "")}</textarea>
...
async function resolveErrorReport(id) {
  if (!confirm("이 오류 제보를 처리 완료로 바꿀까요?")) return;
...
async function markDuplicate(id) {
  const linkedCafeId = prompt("중복 처리할 기존 cafe id를 입력하세요", "") || "";
...
async function saveNotice() {
  ...
  if (!res.ok || !data.ok) throw new Error(data.error || "공지 저장 실패");
  alert("공지 저장 완료");
}
...
async function uploadCsv() {
  try {
    ...
    if (!res.ok || !payload.ok) throw new Error(payload.error || "CSV 업로드 실패");
    ...
    const duplicateIds = (payload.duplicateRows || []).slice(0, 5).map((row) => `${row.id} 중복`).join(", ");
    const duplicateText = duplicated ? ` / 중복 ${duplicated}${duplicateIds ? ` (${duplicateIds}${duplicated > 5 ? " 외" : ""})` : ""}` : "";
...
async function resetCsv() {
  ...
  if (!confirm(`현재 카페 데이터 ${count}개를 전부 삭제합니다.\n삭제 후 CSV 업로드로 다시 넣어야 합니다.\n계속할까요?`)) return;
  const typed = prompt("정말 초기화하려면 아래에 '초기화'라고 입력하세요.", "");
  if (typed !== "초기화") {
    alert("초기화를 취소했습니다.");
    return;
  }
...
function renderUsers() {
  ...
  $("user-list").innerHTML = state.users.map((user) => `
    <div class="item">
      <h3>${esc(user.username)} <small style="font-size:.72rem;">(${esc(user.role)})</small></h3>
...
async function init() {
  ...
  $("hello").textContent = `${state.me.username} (${state.me.role})`;
  ...
  $("save-cafe-btn").addEventListener("click", saveCafe);
  $("delete-btn").addEventListener("click", deleteCafe);
  ...
  $("role-set").addEventListener("click", setRole);
