...
export async function toggleFavorite(req, env) {
  return withGuard(req, env, async () => {
    ...
    const cafe = await env.DB.prepare("SELECT id FROM cafes WHERE id = ?").bind(cafeId).first();
    if (!cafe) throw new HttpError(404, "Cafe not found");

    const exists = await env.DB.prepare("SELECT id FROM favorites WHERE user_id = ? AND cafe_id = ?").bind(user.user_id, cafeId).first();
    const action = cleanText(body.action, 20) || "toggle";

    if ((action === "toggle" && !exists) || action === "add") {
      await env.DB.prepare("INSERT OR IGNORE INTO favorites(id, user_id, cafe_id, created_at) VALUES (?, ?, ?, ?)")
        .bind(crypto.randomUUID(), user.user_id, cafeId, nowIso()).run();
      return json({ ok: true, favored: true }, 200, req, env);
    }

    if ((action === "toggle" && exists) || action === "remove") {
      await env.DB.prepare("DELETE FROM favorites WHERE user_id = ? AND cafe_id = ?")
        .bind(user.user_id, cafeId).run();
